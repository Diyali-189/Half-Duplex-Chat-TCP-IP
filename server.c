	...
	switch(writing)
		...
		case 0:
			while( (n = recv( client_fd, buffer, BUFSIZE, 0)) > 0)
    				...
    				send(client_fd, snd, strlen(snd),0);
    				if(strcmp(buffer,"X") == 10){
    					fprintf(stdout,"Server starts speaking!!\n");
        				writing = 1;
        				break;
        				}
    		...
    		case 1:
    			while(fgets( buffer, BUFSIZE, stdin ) != NULL)
                		...
                		if( n < 0 )
                    		...
                    		recv(client_fd,&clientMessage,sizeof(clientMessage),0);
                		//fprintf(stdout,"%s",clientMessage);
                		fputs(clientMessage,stdout);
                		fprintf(stdout,"\n");
            			if(strcmp(buffer,"X") == 10){
            				fprintf(stdout,"Server stops speaking!!\n");
                			writing = 0;
                			break;
                			}
                		else if(strcmp(buffer,"XX") == 10)
